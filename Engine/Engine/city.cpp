...
void Cityclass::Draw_streets(ID2D1RenderTarget * target, ID2D1Brush* brush)
{
	for (int i = 0; i < n_Streets; i++)
	{
		target->DrawLine(Point2F(Streets[i].firstcor.x, Streets[i].firstcor.y), Point2F(Streets[i].secondcor.x, Streets[i].secondcor.y), brush, 1);
	...
	target->DrawLine(Point2F(Center.x - 5000, Center.y), Point2F(Center.x + 5000, Center.y), brush, 2);
	target->DrawLine(Point2F(Center.x, Center.y - 5000), Point2F(Center.x, Center.y + 5000), brush, 2);
}

int Cityclass::get_n_streets()
{
	return n_Streets;
}

Cityclass::Street * Cityclass::get_streets()
{
	return Streets;
}

D2D1_POINT_2L Cityclass::GetCenter()
{
	return Center;
}
