...
bool SystemClass::PlaceUnit(D2D1_POINT_2F Pos, Object* &Obj, int index, UINT unitType, UINT Fraction, UINT FactoryType, EmployeeClass* own)
{
	...
	else if (unitType == OBJTYPES::FACTORY)
	{
		...
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::Bread, FactoryClass::Flour, 1000, own, FactoryType, salary);
		}
		else if (FactoryType == FactoryClass::factory_type::FARM)
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::Wheat, NULL, 750, own, FactoryType, salary);
		}
		else if (FactoryType == FactoryClass::factory_type::MILL)
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::Flour, FactoryClass::Wheat, 500, own, FactoryType, salary);
		}
		else if (FactoryType == FactoryClass::factory_type::MINE)
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::Iron_ore, NULL, 5000, own, FactoryType, salary);
		}
		else if (FactoryType == FactoryClass::factory_type::FORGER)
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::Steel, FactoryClass::Iron_ore, 3000, own, FactoryType, salary);
		}
		else if (FactoryType == FactoryClass::factory_type::WORKSHOP)
		{
			((FactoryClass*)Obj)->Init_factory(FactoryClass::cutlery, FactoryClass::Steel, 8000, own, FactoryType, salary);
		}
